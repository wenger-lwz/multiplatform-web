      ...
      >
        <template #bodyCell="{ record, column }">
          <template v-if="column.dataIndex === 'sku'">
            <gj-radio v-model="selectCode" :value="record.sku"
              ><div class="fontNoWrap-1">{{ record.sku }}</div></gj-radio
            >
          </template>
          <template v-if="column.dataIndex === 'skuName'">
            <div class="fontNoWrap-1">{{ record.skuName }}</div>
          </template>
        </template>
      </gj-table>
    </div>
...
import { skuInfoList } from "@/api/published/materialCenter";
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const searchSku = () => {
      skuInfoList({ sku: data.skuCode }).then((res) => {
