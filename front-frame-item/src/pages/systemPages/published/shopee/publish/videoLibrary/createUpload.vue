        ...
        <div class="hot-text">
          <ul>
            <li class="hot-ico">
              ...
              ></span>
              1.仅支持上传mp4格式视频，且视频限制30M以内；
            </li>
            <li>2.分辨率不超过1280x1280px，视频市场在10~60s内。</li>
          </ul>
        </div>
        <div class="imgbox">
          <div
          ...
          <div class="upload-btn image-box">
            <gj-button type="primary" shape="round" @click="openModel('sp')"
              >添加视频
            </gj-button>
          </div>
        </div>
...
import { saveShopeeVideoInfo } from "@api/published/shopee.js";
import { getListingCommonShop } from "@/api/published/common";
import SvgIcon from "@/components/SvgIcon";
import { GjMessage } from "@gj/atom";
export default defineComponent({
  name: "PubShopeeVideoLibraryCreateUpload",
  ...
  setup(props, { emit }) {
    ...
    const confirm = (val) => {
      ...
      if (data.type == 1) {
        data.videList.push(val);
      } else {
        val.forEach((element) => {
          data.videList.push(element);
        });
        data.videoVisible = false;
      }
    };
