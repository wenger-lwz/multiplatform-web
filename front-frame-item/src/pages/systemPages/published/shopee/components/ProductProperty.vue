...
export default {
  ...
  setup(props) {
    ...
    const getqueryEbayCategoryAttribute = () => {
      if (
        !store.getters.getParams.basicInfo.siteId ||
        !store.state.params.params.productInfo.platformCategoryId
      )
        return;
      let params = {
        /* site: 'MY',
        ...
        categoryId: store.state.params.params.productInfo.platformCategoryId,
      };
      queryShopeeCategoryAttribute(params).then((res) => {
        ...
        if (res.data && res.data.length > 0) {
          if (res.data.optionalAttributeList.length > 0) {
            data.productPropertyInfo.optionArr =
              res.data.optionalAttributeList.map((item) => {
                return {
                  ...item,
    ...
    const setPageData = (aData) => {
      if (aData.attributes && aData.attributes.length > 0) {
        data.productPropertyInfo.optionArr = aData.attributes.map((item) => {
          return {
            display_attribute_name: item.arrtName,
            page: item.attrValueList.map((it) => {
              return {
                value_id: it.attrValueId,
                display_value_name: it.attrValue,
              };
            }),
            moduVal: item.moduVal,
            display_value_name: item.display_value_name,
          };
        });
      }
    };
    ...
    watch(
      () => store.state.params.params.productInfo,
      (newVal) => {
        console.log(
          store.state.params.params.productInfo.platformCategoryId,
          "newVal"
