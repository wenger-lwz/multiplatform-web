  ...
  setup(props) {
    ...
    const setEntry = () => {
      if (data.selectedRowKeys.length <= 0) {
        return GjMessage.warning("请选择要批量录入的数据");
      }
    ...
    const setPageData = (aData) => {
      ...
      const tempTableData = [];
      if (aData.variants && aData.variants.length > 0) {
        aData.variants?.forEach((item) => {
          // const dcrItem = []; // 删除需用于匹配
          const tableItem = {
            ...item,
          };
        ...
        data.availableGroup = {};
        data.totalCartesianProduct = JSON.parse(
