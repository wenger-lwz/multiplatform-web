  ...
  <layout>
    <template #seach>
      ...
      <gj-row class="rowHeightStyle">
        <gj-space>
          <gj-select
            v-model="formDataSearch.stationCode"
            placeholder="站点"
            size="small"
            allow-clear
            allow-search
            @change="searchSiteId"
            @popup-visible-change="getSiteIdList"
            :loading="siteIdLoading"
            class="flexItem"
          >
            <gj-option
              v-for="item in sitList.children"
              :key="item.codeId"
              :label="item.name"
              :value="item.name"
            >
          ...
          <gj-select
            size="small"
            placeholder="发布人"
            v-model="formDataSearch.publisherId"
            allow-clear
            allow-search
            @change="searchPublisher"
            @popup-visible-change="getPublisherList"
            :loading="publisherLoading"
            class="flexItem"
          >
            <gj-option
              v-for="(item, index) in publisherList.children"
              :key="index"
              :label="item.name"
              :value="item.name"
            ></gj-option>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    // 获取全部站点数据
    const getSiteIdList = (val) => {
      if (val && data.sitList.children.length == 0) {
        data.siteIdLoading = true;
        data.sitList.children = [
          { name: "全部", codeId: "All" },
          { name: "美国", codeId: "US" },
          { name: "eBay汽车", codeId: "eBayMotors" },
          { name: "英国", codeId: "Britain" },
          { name: "德国", codeId: "Germany" },
          { name: "法国", codeId: "France" },
          { name: "意大利", codeId: "Italy" },
          { name: "西班牙", codeId: "Spain" },
          { name: "加拿大(英语)", codeId: "CanadaEnglish" },
          { name: "加拿大(法语)", codeId: "CanadaFrance" },
          { name: "澳大利亚", codeId: "Australia" },
        ];
        setTimeout(() => {
          data.siteIdLoading = false;
        }, 200);
      }
    };
    ...
    // 获取全部发布人数据
    const getPublisherList = (val) => {
      if (val && data.publisherList.children.length == 0) {
        data.publisherLoading = true;
        data.publisherList.children = [
          {
            name: "全部发布人",
            value: "0",
            codeId: "All",
    ...
    // 搜索站点数据
    const searchSiteId = (e) => {
      setActivetData(data.sitList, e);
    };
    // 搜索店铺数据
    ...
    const searchPublisher = (e) => {
      setActivetData(data.publisherList, e);
    };
    // 搜索负责人数据
    ...
    const getList = () => {
      ...
      props
        ...
        .finally(() => {
          ...
          data.form.tableData = [{ name: "kkkkkk", sited: "kkkkss" }];
        });
    };
    ...
    const closeTags = (val) => {
      if (val.value.name === "站点") {
        data.formDataSearch.stationCode = "";
      } else if (val.value.name === "店铺") {
      ...
      } else if (val.value.name === "发布人") {
        data.formDataSearch.publisherId = "";
