<template>
  ...
  <gj-select
    v-if="showItem.includes('videoStatus')"
    v-model="searchData.videoStatus"
    class="search-select"
    placeholder="状态"
    @change="selectDataChange('videoStatus', '状态')"
  >
    <gj-option
      v-for="item in videoStatusListOpt"
      :key="item.value"
      :value="item.value"
      >{{ item.label }}</gj-option
    >
...
import { formatDateTime, setSearchValue } from "@/utils/format";
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const getShowValue = (searchDataId) => {
      ...
      switch (searchDataId) {
        ...
        case "statusId":
          ...
          break;
        case "videoStatus":
          item = videoStatusListOpt.find(
            (it) => it.value === data.searchData.videoStatus
          );
          value = item?.label;
          break;
        case "specsType":
    ...
    const handleChangeObject = (searchDataId, searchDataName, value = []) => {
      ...
      const searchItem = {
        showName: searchDataName,
        showValue: setSearchValue(showValue),
        id: searchDataId,
        splitSymbol: data.splitSymbol,
      };
      emit("filter-change", data.searchData);
