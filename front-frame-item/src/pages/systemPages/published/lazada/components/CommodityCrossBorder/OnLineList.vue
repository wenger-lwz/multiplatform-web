<template>
  <CommonPage
    :pageRest="true"
    currentRouterPermission="LZD/commodityCrossBorder/online/view"
    ...
    <template #operate_bar-content>
      <OperateBar>
        <template #left>
          <gj-button
            type="plain"
            v-permissions="['LZD/commodityCrossBorder/online/patchSync']"
            @click="patchSync"
            >批量同步</gj-button
          >
      ...
      return [
        {
          ...
          outside: true,
          isPopConfirm: false,
          permission: ["LZD/commodityCrossBorder/online/sync"],
          click: (row) => {
            if (data.loadingMap[row.id]) {
        ...
        customRender: ({ record }) => {
          ...
          return (
            <div>
              ...
              <div>{copyNode}</div>
              <div class="salePrice-tag-box">{elem}</div>
            </div>
          );
        },
      },
