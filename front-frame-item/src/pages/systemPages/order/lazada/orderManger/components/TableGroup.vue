<template>
  <!-- 自定义列 -->
  <gb-custom-column
    :id="'LAZADA_lazadaAllOrder_table'"
  ...
  >
    <template #bodyCell="{ column, record }">
      ...
      <!-- 取消状态 -->
      <template v-if="column.dataIndex === 'orderCancelStatus'">
        <div>{{ record?.orderCancelStatus || "-" }}</div>
      </template>
      <!-- so单 -->
      ...
      <template v-if="column.dataIndex === 'action'">
        <gj-button
          v-permissions="['LZD/lazadaAllOrder/info']"
...
import { getOrderList } from "@/api/order/lazada";
