<template>
  <CommonPage
    ...
    :currentRouterPermission="'LZD/lazadaAllOrder/view'"
  >
    <!-- 默认视图 -->
    <template #tag-content>
      <gb-default-view
        :id="'LAZADA_lazadaAllOrder_view'"
        :code="0"
        :get-list="getList"
        :post-list="postList"
        :query-map="queryMap"
    ...
    <template #table-content>
      <TableGroup ref="orderLazadaTable"></TableGroup>
    </template>
  </CommonPage>
</template>
...
import { getRefundStatus, getCancelStatus } from "@/api/order/lazada";
export default defineComponent({
  ...
  setup() {
    ...
    const popconfirmOk = () => {
      ...
      proxy.$refs.orderLazadaFilter?.advanceSearchChange(
        data.queryCondition,
        data.option
      );
    };
    ...
    // 设置自定义列
    const settingColumn = () => {
      proxy.$refs.orderLazadaTable?.setting();
    };

    // 搜索关联table
    const filterChange = (data) => {
      proxy.$refs.orderLazadaTable?.getData(data);
    };
    // 重置
    ...
    const tagChange = (params) => {
      ...
      proxy.$refs.orderLazadaTable?.$refs?.commonTableRef?.calculateWH();
    };
    ...
    const delQueryCondition = (val) => {
      ...
      proxy.$refs.orderLazadaFilter?.changeFilterCondition(val);
      // 计算table高度
      proxy.$refs.orderLazadaTable?.$refs?.commonTableRef?.calculateWH();
    };
    // 保存视图回显
    const changeQueryList = (val) => {
      ...
      // 1、重置方法
      resetAdvanceSearch();
      proxy.$refs.orderLazadaTable?.reset();
      if (val && val.length) {
        val.forEach((item) => {
        ...
        proxy.$refs.orderLazadaFilter?.reset(false);
        // 2、重新赋值
        proxy.$refs.orderLazadaFilter?.changeQueryList(val);
      } else {
        proxy.$refs.orderLazadaFilter?.reset(true);
      }
      // 3.计算table高度
      proxy.$refs.orderLazadaTable?.$refs?.commonTableRef?.calculateWH();
    };
