<template>
  <CommonPage
    ...
    :currentRouterPermission="'ALI/aliAllOrder/view'"
  >
    <!-- 默认视图 -->
    <template #tag-content>
      <gb-default-view
        :id="'ALI_aliAllOrder_view'"
        :code="0"
        :get-list="getList"
        :post-list="postList"
        :query-map="queryMap"
    ...
    <template #table-content>
      <TableGroup ref="orderAliTable"></TableGroup>
    </template>
  </CommonPage>
</template>
...
import { getRefundStatus, getCancelStatus } from "@/api/order/aliExpress";
export default defineComponent({
  ...
  setup() {
    ...
    const popconfirmOk = () => {
      ...
      proxy.$refs.orderAliFilter?.advanceSearchChange(
        data.queryCondition,
        data.option
      );
    };
    ...
    // 设置自定义列
    const settingColumn = () => {
      proxy.$refs.orderAliTable?.setting();
    };

    // 搜索关联table
    const filterChange = (data) => {
      proxy.$refs.orderAliTable?.getData(data);
    };
    ...
    const tagChange = (params) => {
      ...
      proxy.$refs.orderAliTable?.$refs?.commonTableRef?.calculateWH();
    };
    ...
    const delQueryCondition = (val) => {
      ...
      proxy.$refs.orderAliFilter?.changeFilterCondition(val);
      // 计算table高度
      proxy.$refs.orderAliTable?.$refs?.commonTableRef?.calculateWH();
    };
    ...
    const changeQueryList = (val) => {
      ...
      // 1、重置方法
      resetAdvanceSearch();
      proxy.$refs.orderAliTable?.reset();
      if (val && val.length) {
        val.forEach((item) => {
        ...
        proxy.$refs.orderAliFilter?.reset(false);
        // 2、重新赋值
        proxy.$refs.orderAliFilter?.changeQueryList(val);
      } else {
        proxy.$refs.orderAliFilter?.reset(true);
      }
      // 3.计算table高度
      proxy.$refs.orderAliTable?.$refs?.commonTableRef?.calculateWH();
    };
