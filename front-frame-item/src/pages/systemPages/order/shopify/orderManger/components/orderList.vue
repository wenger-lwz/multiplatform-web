<template>
  <CommonPage
    ...
    :currentRouterPermission="'SHPY/shopifyAllOrder/view'"
  >
    <!-- 默认视图 -->
    <template #tag-content>
      <gb-default-view
        :id="'SHOPIFY_shopifyAllOrder_view'"
        :code="0"
        :get-list="getList"
        :post-list="postList"
        :query-map="queryMap"
    ...
    <template #table-content>
      <TableGroup ref="orderShopifyTable"></TableGroup>
    </template>
  </CommonPage>
</template>
...
import { getRefundStatus, getCancelStatus } from "@/api/order/shopify";
export default defineComponent({
  ...
  setup() {
    ...
    const popconfirmOk = () => {
      ...
      proxy.$refs.orderShopifyFilter?.advanceSearchChange(
        data.queryCondition,
        data.option
      );
    };
    ...
    // 设置自定义列
    const settingColumn = () => {
      proxy.$refs.orderShopifyTable?.setting();
    };

    // 搜索关联table
    const filterChange = (data) => {
      proxy.$refs.orderShopifyTable?.getData(data);
    };
    ...
    const tagChange = (params) => {
      console.log(params, "data.queryMap");
      Object.assign(data.queryMap, params);
      proxy.$refs.orderShopifyTable?.$refs?.commonTableRef?.calculateWH(); // 计算table高度
    };
    ...
    const delQueryCondition = (val) => {
      ...
      // 搜索及参数处理
      proxy.$refs.orderShopifyFilter?.changeFilterCondition(val);
      proxy.$refs.orderShopifyTable?.$refs?.commonTableRef?.calculateWH(); // 计算table高度
    };
    ...
    const changeQueryList = (val) => {
      ...
      // 1、重置方法
      resetAdvanceSearch();
      proxy.$refs.orderShopifyTable?.reset();
      if (val && val.length) {
        val.forEach((item) => {
        ...
        proxy.$refs.orderShopifyFilter?.reset(false);
        // 2、重新赋值
        proxy.$refs.orderShopifyFilter?.changeQueryList(val);
      } else {
        proxy.$refs.orderShopifyFilter?.reset(true);
      }
      // 3、计算table高度
      proxy.$refs.orderShopifyTable?.$refs?.commonTableRef?.calculateWH();
    };
