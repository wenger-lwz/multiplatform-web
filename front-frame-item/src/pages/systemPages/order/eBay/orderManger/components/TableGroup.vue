  ...
  >
    <template #bodyCell="{ column, record }">
      ...
      <template v-if="column.dataIndex === 'erpShopName'">
        <div v-if="record?.erpShopName || record?.regionName">
          <gb-text>
            <div>
              {{ record?.erpShopName }}
              <span v-show="record?.regionName">
                {{ "：" + record?.regionName }}
              </span>
            </div>
          </gb-text>
        </div>
        <span v-show="!(record?.erpShopName || record?.regionName)">-</span>
      </template>
      <!-- 平台订单状态/取消状态 -->
      ...
      <template v-if="column.dataIndex === 'action'">
        <gj-button
          v-permissions="['EBAY/eBayAllOrder/info']"
    ...
    <!-- 总计 -->
    <!-- <template #summary>
      <gj-table-summary-row>
        <gj-table-summary-cell :index="0" :col-span="2">
          <span>总计</span>
        </gj-table-summary-cell>
        <gj-table-summary-cell column-key="buyTotalCount">
          <template #default>{{ summaryData.buyTotalCount }}</template>
        </gj-table-summary-cell>
        <gj-table-summary-cell column-key="totalAmount">
          <template #default>{{ summaryData.totalAmount }}</template>
        </gj-table-summary-cell>
        <gj-table-summary-cell column-key="buyerPayAmount">
          <template #default>{{ summaryData.buyerPayAmount }}</template>
        </gj-table-summary-cell>
      </gj-table-summary-row>
    </template> -->
  </CommonTable>
...
import { getOrderList } from "@/api/order/ebay";
      ...
      columnList: [
        // 自定义表格默认展示列
        {
          ...
          children: [
            ...
            {
              dataIndex: "orderRefundStatusName",
              label: "退款状态/退款总金额",
              isSelected: true,
              isFixed: false,
            },
