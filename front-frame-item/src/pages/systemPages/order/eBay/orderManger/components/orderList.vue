<template>
  <CommonPage
    ...
    :currentRouterPermission="'EBAY/eBayAllOrder/view'"
  >
    <!-- 默认视图 -->
    <template #tag-content>
      <gb-default-view
        :id="'EBAY_eBayAllOrder_view'"
        :code="0"
        :get-list="getList"
        :post-list="postList"
        :query-map="queryMap"
    ...
    <template #table-content>
      <TableGroup ref="orderEbayTable"></TableGroup>
    </template>
  </CommonPage>
</template>
...
import { getRefundStatus, getCancelStatus } from "@/api/order/ebay";
...
export default defineComponent({
  ...
  setup() {
    ...
    const popconfirmOk = () => {
      ...
      proxy.$refs.orderEbayFilter?.advanceSearchChange(
        data.queryCondition,
        data.option
      );
    };
    ...
    // 设置自定义列
    const settingColumn = () => {
      proxy.$refs.orderEbayTable?.setting();
    };

    // 搜索关联table
    const filterChange = (data) => {
      proxy.$refs.orderEbayTable?.getData(data);
    };
    ...
    const tagChange = (params) => {
      ...
      proxy.$refs.orderEbayTable?.$refs?.commonTableRef?.calculateWH();
    };
    ...
    const delQueryCondition = (val) => {
      ...
      proxy.$refs.orderEbayFilter?.changeFilterCondition(val);
      // 计算table高度
      proxy.$refs.orderEbayTable?.$refs?.commonTableRef?.calculateWH();
    };
    ...
    const changeQueryList = (val) => {
      ...
      // 1、重置方法
      resetAdvanceSearch();
      proxy.$refs.orderEbayTable?.reset();
      if (val && val.length) {
        val.forEach((item) => {
        ...
        proxy.$refs.orderEbayFilter?.reset(false);
        // 2、重新赋值
        proxy.$refs.orderEbayFilter?.changeQueryList(val);
      } else {
        proxy.$refs.orderEbayFilter?.reset(true);
      }
      // 3.计算table高度
      proxy.$refs.orderEbayTable?.$refs?.commonTableRef?.calculateWH();
    };
