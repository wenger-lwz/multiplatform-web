<template>
  ...
  <gb-select
    ...
    multiple
    allow-clear
    v-model="queryCondition.regionIds"
    @confirm="multipleSearch('regionIds')"
    @clear="multipleSearch('regionIds')"
    @remove="multipleSearch('regionIds')"
  >
    <gb-select-option
      v-for="item in siteList"
      :key="item.regionCode"
      :value="item.regionCode"
      >{{ item.regionName }}</gb-select-option
    >
  </gb-select>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
      ...
      nextTick(() => {
        ...
        data.queryCondition[dataEnd] =
          val && val.length
            ...
            : "";
        console.log(data.queryCondition, "data.queryCondition");
        let text = "";
        if (flag) {
          text =
    ...
    // 下拉多选 （val：value值, dataList：list列表, name：给tag的key, modelName：绑定的字段, value：list的valueName, label:list的labelName, isAdvance:是否是高级搜索）
    ...
    const toGetRefundStatus = async () => {
      ...
      data.refundStatusList = res.data;
    };

    // 获取取消状态
    const toGetCancelStatus = async () => {
      const res = await getCancelStatus();
      data.cancelStatusList = res.data;
    };
