...
function layout(data, _bind_) {
  const item = function (list) {
    ...
    list.forEach((item, index) => {
      html += `<el-col id='${data.key}_${index}' :span='${
        item.span
      }'>${__fn_build__(
        item.list,
        _bind_,
  ...
  return `<el-row id='${data.key}' :gutter='${data.gutter}'>${item(
    data.drag
  )}</el-row>`;
}
...
function input(data, _bind_) {
  ...
  return `<el-form-item size='${data.size}' prop='${data.value}' label='${
    data.name
  }'><el-input ${
    bind ? 'v-model="' + pre + data.value + '"' : ""
  } style='width: ${data.width}' id='${data.key}' ${
    data.disabled ? "disabled" : ""
  } placeholder='${data.placeholder}'/></el-form-item>`;
}
__map__.input = input;
...
function select(data, _bind_) {
  const item = function (list) {
    let html = "";
    list.forEach((item) => {
      html += `<el-option label='${item.label}' ${
  ...
  return `<el-form-item size='${data.size}' prop='${data.value}' label='${
    ...
    data.disabled ? "disabled" : ""
  } placeholder='${data.placeholder}' value=''>${item(
    data.option
...
function date(data, _bind_) {
  ...
  return `<el-form-item size='${data.size}' prop='${data.value}' label='${
    ...
    data.disabled ? "disabled" : ""
  } type="date" placeholder='${data.placeholder}'/></el-form-item>`;
}
__map__.date = date;
...
function _switch(data, _bind_) {
  ...
  return `<el-form-item  size='${data.size}' prop='${data.value}' label='${
    data.name
  }'><el-switch id='${data.key}' ${
    bind ? 'v-model="' + pre + data.value + '"' : ""
  } ${data.disabled ? "disabled" : ""}/></el-form-item>`;
}
...
function checkbox(data, _bind_) {
  const item = function (list) {
    let html = "";
    list.forEach((item, index) => {
      html += `<el-checkbox id='${data.key}_${index}' ${
        data.disabled ? "disabled" : ""
  ...
  return `<el-form-item  size='${data.size}' prop='${data.value}' label='${
    data.name
  }'><el-checkbox-group id='${data.key}' ${
    bind ? 'v-model="' + pre + data.value + '"' : ""
  }>${item(data.option)}</el-checkbox-group></el-form-item>`;
}
...
function radio(data, _bind_) {
  const item = function (list) {
    let html = "";
    list.forEach((item, index) => {
      html += `<el-radio id='${data.key}_${index}' ${
        data.disabled ? "disabled" : ""
  ...
  return `<el-form-item  size='${data.size}' prop='${data.value}' label='${
    data.name
  }'><el-radio-group id='${data.key}' ${
    bind ? 'v-model="' + pre + data.value + '"' : ""
  }>${item(data.option)}</el-radio-group></el-form-item>`;
}
__map__.radio = radio;
...
function textarea(data, _bind_) {
  ...
  return `<el-form-item size='${data.size}' prop='${data.value}' label='${
    data.name
  }'><el-input resize='none' id='${data.key}' ${
    bind ? 'v-model="' + pre + data.value + '"' : ""
  } style='width: ${data.width}' type="textarea" placeholder='${
    data.placeholder
  }'/></el-form-item>`;
}
...
class encoder {
  ...
  build(data, preview) {
    ...
    return {
      template: `<el-container><el-form ref='formData' :rules='rules' style='width: 100%' :model='${pre.substr(
        0,
        pre.length - 1
      )}' label-width='80px'>${__fn_build__(
        data,
        local,
        __fn_bind__,
        __fn_build__
