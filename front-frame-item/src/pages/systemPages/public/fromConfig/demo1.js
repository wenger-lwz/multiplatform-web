export default {
  ...
  viewer: {
    ...
    table: {
      template:
        '<el-table :ref="item.key" :size="item.size" border :style="{width: (item.width === \'100%\' ? \'calc(\' + item.width + \' - 6px)\':item.width), margin: \'3px\'}"> \
    ...
    table: function (data, _bind_, _fn_bind_) {
      const item = function (list) {
        ...
        list.forEach((item) => {
          ...
          } fixed="${item.fixed}" label="${item.label}" width="${
            item.width
          }" />`;
        });
        return html;
      ...
      return `<el-table size="${data.size}" border ${
        bind ? ':data="data.' + data.value + '"' : ""
      } style="width: ${data.width}">${item(data.column)}</el-table>`;
    },
  },
};
