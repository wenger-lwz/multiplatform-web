...
import {
  platformList,
  columnsList,
} from "@pagesSystem/customerService/infrastructure/config/classificationLabelList.js";
import addClassificationLabelForm from "@pagesSystem/customerService/infrastructure/components/addClassificationLabelForm.vue";
import OperateBar from "@/components/OperateBar.vue";
import {
  getClassificationLabelList,
  addClassificationLabel,
  editClassificationLabel,
  delClassificationLabel,
  disableOrEnableLabel,
...
export default defineComponent({
  ...
  setup() {
    ...
    const handelLabel = (row, type) => {
      ...
      data.fromTitle = type === 1 ? "编辑分类标签" : "添加分类标签";
    };
    //  操作列数据
      ...
      proxy.$refs["createFrom"].submitForm().then(async (res) => {
        ...
        if (res === "200") {
          ...
          if (data.fromType === 1) {
            ...
            // 编辑
            await editClassificationLabel(params);
            GjMessage.success("修改成功");
            data.fromDialogVisible = false;
            searchTable();
          } else {
            // 添加
            await addClassificationLabel(params);
    ...
    const delLabel = async (params) => {
      await delClassificationLabel(params);
    ...
    const searchTable = () => {
      ...
      getClassificationLabelList(params)
        .then((res) => {
    ...
    const setAllocationRule = async (val) => {
      ...
      await disableOrEnableLabel(params);
